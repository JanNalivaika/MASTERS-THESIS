...
for rot_winkel in range(1):
    for kipp_winkel in range(-50, 50):
        ...
        for selection in range(1, 4):
            ...
            if selection == 1:
                x = np.cos(np.deg2rad(iter)) * (300 - iter / 10) + iter/10
                y = np.sin(np.deg2rad(iter)) * (300 - iter / 10) - iter/10
                z = iter / 6
                tit = "Converging-Diverging Spiral"

            if selection == 2:
                y = np.cos(np.deg2rad(iter)) * (400 - iter / 5)
                x = np.cos(np.deg2rad(iter)) * np.sin(np.deg2rad(iter)) * (500 - iter / 6)
                z = iter / 6
                tit = "Converging Loop"
            ...
            if selection == 3:
                ...
                y = iter / 3 - (2500 / 3 / 2)
                z = np.sin(np.deg2rad(x)) * 100 + (iter**2)/35000
                tit = "Pendulum Wave"
